# script is auto-generated at 2025-10-20T09:20:38.380800
#
...
from . import lib
from .mosquitto import bind, bind_all
...
def bind_all(lib, ctx):
    ...
    bind(C.c_int, lib.mosquitto_lib_version, ctx.c_int_p, ctx.c_int_p, ctx.c_int_p)
    # int mosquitto_lib_init(void);
    bind(C.c_int, lib.mosquitto_lib_init)
    # int mosquitto_lib_cleanup(void);
    bind(C.c_int, lib.mosquitto_lib_cleanup)
    # struct mosquitto *mosquitto_new(const char *id, bool clean_session, void *obj);
    bind(ctx.c_mosquitto_p, lib.mosquitto_new, C.c_char_p, C.c_bool, C.c_void_p)
    # void mosquitto_destroy(struct mosquitto *mosq);
    bind(None, lib.mosquitto_destroy, ctx.c_mosquitto_p)
    ...
    bind(
        ...
        C.c_int,
        C.c_bool,
        ctx.c_mosquitto_property_p,
    )
    # int mosquitto_will_clear(struct mosquitto *mosq);
    bind(C.c_int, lib.mosquitto_will_clear, ctx.c_mosquitto_p)
    # int mosquitto_username_pw_set(struct mosquitto *mosq, const char *username, const char *password);
    bind(
        C.c_int,
        lib.mosquitto_username_pw_set,
        ctx.c_mosquitto_p,
        C.c_char_p,
        C.c_char_p,
    )
    ...
    bind(
        C.c_int, lib.mosquitto_connect, ctx.c_mosquitto_p, C.c_char_p, C.c_int, C.c_int
    )
    ...
    bind(C.c_int, lib.mosquitto_reconnect, ctx.c_mosquitto_p)
    # int mosquitto_reconnect_async(struct mosquitto *mosq);
    bind(C.c_int, lib.mosquitto_reconnect_async, ctx.c_mosquitto_p)
    # int mosquitto_disconnect(struct mosquitto *mosq);
    bind(C.c_int, lib.mosquitto_disconnect, ctx.c_mosquitto_p)
    ...
    bind(C.c_int, lib.mosquitto_unsubscribe, ctx.c_mosquitto_p, ctx.c_int_p, C.c_char_p)
    ...
    bind(
        ...
        ctx.c_mosquitto_message_p,
        ctx.c_mosquitto_message_p,
    )
    # void mosquitto_message_free(struct mosquitto_message **message);
    bind(None, lib.mosquitto_message_free, ctx.c_mosquitto_message_pp)
    # void mosquitto_message_free_contents(struct mosquitto_message *message);
    bind(None, lib.mosquitto_message_free_contents, ctx.c_mosquitto_message_p)
    # int mosquitto_loop_forever(struct mosquitto *mosq, int timeout, int max_packets);
    bind(C.c_int, lib.mosquitto_loop_forever, ctx.c_mosquitto_p, C.c_int, C.c_int)
    # int mosquitto_loop_start(struct mosquitto *mosq);
    bind(C.c_int, lib.mosquitto_loop_start, ctx.c_mosquitto_p)
    # int mosquitto_loop_stop(struct mosquitto *mosq, bool force);
    bind(C.c_int, lib.mosquitto_loop_stop, ctx.c_mosquitto_p, C.c_bool)
    # int mosquitto_loop(struct mosquitto *mosq, int timeout, int max_packets);
    bind(C.c_int, lib.mosquitto_loop, ctx.c_mosquitto_p, C.c_int, C.c_int)
    # int mosquitto_loop_read(struct mosquitto *mosq, int max_packets);
    bind(C.c_int, lib.mosquitto_loop_read, ctx.c_mosquitto_p, C.c_int)
    # int mosquitto_loop_write(struct mosquitto *mosq, int max_packets);
    bind(C.c_int, lib.mosquitto_loop_write, ctx.c_mosquitto_p, C.c_int)
    # int mosquitto_loop_misc(struct mosquitto *mosq);
    bind(C.c_int, lib.mosquitto_loop_misc, ctx.c_mosquitto_p)
    # int mosquitto_socket(struct mosquitto *mosq);
    bind(C.c_int, lib.mosquitto_socket, ctx.c_mosquitto_p)
    # bool mosquitto_want_write(struct mosquitto *mosq);
    bind(C.c_bool, lib.mosquitto_want_write, ctx.c_mosquitto_p)
    # int mosquitto_threaded_set(struct mosquitto *mosq, bool threaded);
    bind(C.c_int, lib.mosquitto_threaded_set, ctx.c_mosquitto_p, C.c_bool)
    # int mosquitto_opts_set(struct mosquitto *mosq, enum mosq_opt_t option, void *value);
    bind(
        C.c_int, lib.mosquitto_opts_set, ctx.c_mosquitto_p, ctx.c_mosq_opt_t, C.c_void_p
    )
    # int mosquitto_int_option(struct mosquitto *mosq, enum mosq_opt_t option, int value);
    bind(
        C.c_int, lib.mosquitto_int_option, ctx.c_mosquitto_p, ctx.c_mosq_opt_t, C.c_int
    )
    ...
    bind(C.c_int, lib.mosquitto_max_inflight_messages_set, ctx.c_mosquitto_p, C.c_uint)
    # void mosquitto_message_retry_set(struct mosquitto *mosq, unsigned int message_retry);
    bind(None, lib.mosquitto_message_retry_set, ctx.c_mosquitto_p, C.c_uint)
    # void mosquitto_user_data_set(struct mosquitto *mosq, void *obj);
    bind(None, lib.mosquitto_user_data_set, ctx.c_mosquitto_p, C.c_void_p)
    # void *mosquitto_userdata(struct mosquitto *mosq);
    bind(C.c_void_p, lib.mosquitto_userdata, ctx.c_mosquitto_p)
    # int mosquitto_tls_insecure_set(struct mosquitto *mosq, bool value);
    bind(C.c_int, lib.mosquitto_tls_insecure_set, ctx.c_mosquitto_p, C.c_bool)
    ...
    bind(C.c_void_p, lib.mosquitto_ssl_get, ctx.c_mosquitto_p)
    ...
    bind(C.c_int, lib.mosquitto_string_to_command, C.c_char_p, ctx.c_int_p)
    # int mosquitto_sub_topic_tokenise(const char *subtopic, char ***topics, int *count);
    bind(
        C.c_int,
        lib.mosquitto_sub_topic_tokenise,
        C.c_char_p,
        ctx.c_char_ppp,
        ctx.c_int_p,
    )
    # int mosquitto_sub_topic_tokens_free(char ***topics, int count);
    bind(C.c_int, lib.mosquitto_sub_topic_tokens_free, ctx.c_char_ppp, C.c_int)
    # int mosquitto_topic_matches_sub(const char *sub, const char *topic, bool *result);
    bind(C.c_int, lib.mosquitto_topic_matches_sub, C.c_char_p, C.c_char_p, ctx.c_bool_p)
    ...
    bind(C.c_int, lib.mosquitto_property_identifier, ctx.c_mosquitto_property_p)
    # const mosquitto_property *mosquitto_property_next(const mosquitto_property *proplist);
    bind(
        ctx.c_mosquitto_property_p,
        lib.mosquitto_property_next,
        ctx.c_mosquitto_property_p,
    )
    ...
    bind(None, lib.mosquitto_property_free_all, ctx.c_mosquitto_property_pp)
    ...
    bind(C.c_int, lib.mosquitto_property_check_all, C.c_int, ctx.c_mosquitto_property_p)
